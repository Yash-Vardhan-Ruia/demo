...
export function KanbanBoard({ pageId = "default" }) {
  const [boardName, setBoardName] = useState("Untitled Kanban Board");
  ...
  const saveKanbanBoard = async () => {
    const payload = { id: pageId, name: boardName, data };
    const { error } = await supabase.from("api.kanban").upsert(payload); // Added "api." prefix
    if (error) console.error("Save kanban board error:", error);
    else alert("Kanban board saved!");
  };
    ...
    <div className="p-4">
      ...
      <div className="flex items-center justify-between mb-4 border-b pb-2">
        <Input
          ...
          className="max-w-xs"
        />
        <Button onClick={saveKanbanBoard}>Save</Button>
      </div>
